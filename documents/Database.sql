...
-- -----------------------------------------------------
-- Table `surfrider`.`spot`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `surfrider`.`spot` (
  `id` INT NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `surfrider`.`session` (
  ...
  INDEX `fk_session_spot1_idx` (`spot_id` ASC) VISIBLE,
  CONSTRAINT `fk_post_user_id`
    FOREIGN KEY (`user_id`)
    REFERENCES `surfrider`.`user` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_session_spot1`
    FOREIGN KEY (`spot_id`)
    REFERENCES `surfrider`.`spot` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
